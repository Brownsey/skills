...
When delegation is available and authorised, use it for every non-trivial implementation. At kickoff, discover the runtime's current concurrency capacity rather than assuming a fixed worker count. Read capacity from runtime instructions or the coordination-tool inventory. If no numeric limit is exposed, use successful dispatches to identify available slots; never infer zero capacity from missing metadata. Stop after the first capacity refusal and retry only after a slot completes. Before the lead writes production feature code, map the requirement dependency graph, stabilise the smallest contracts needed for parallel work, and partition the work into the maximum useful set of non-overlapping packets that can start. Dispatch every ready packet up to the available capacity; if the runtime provides more or effectively unlimited workers, scale to the number of useful independent packets rather than applying an artificial cap. If frontend and backend are separable, they must have separate implementation owners; split additional independent UI, API, data, research and test surfaces while their file ownership and runtime state remain disjoint.
