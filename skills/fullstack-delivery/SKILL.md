...
When delegation is available and authorised, use it for every non-trivial implementation. At kickoff, discover the runtime's current per-session concurrency capacity rather than assuming a fixed worker count. Prefer the capacity exposed by runtime instructions or the coordination-tool inventory; if neither exposes it, inspect the local `agents.max_concurrent_threads_per_session` setting when available. Treat the effective capacity as a ceiling, subtract currently active subagents, and dispatch `min(free slots, ready independent packets)`. If no numeric limit is exposed, use successful dispatches to identify available slots; stop after the first capacity refusal and retry only after a slot completes. Recalculate free capacity after every completion or new dispatch. Stay within the current session and never create or fork peer tasks to multiply its capacity.

Before the lead writes production feature code, map the requirement dependency graph, stabilise the smallest contracts needed for parallel work, and partition the work into the maximum useful set of non-overlapping packets that can start. Dispatch every ready packet up to the available capacity; if the runtime provides more or effectively unlimited workers, scale to the number of useful independent packets rather than applying an artificial cap. If frontend and backend are separable, they must have separate implementation owners; split additional independent UI, API, data, research and test surfaces while their file ownership and runtime state remain disjoint.

The lead retains requirements, shared contracts, dependency and resource locks, integration decisions, agent coordination, deployment and final evidence. It does not own a feature surface merely to keep work local. Keep the lead available for integration and coordination, fill all other currently available capacity whenever enough independent packets are ready, and immediately reuse completed capacity for the next implementation packet, the matching independent validator or the final reviewer. Recheck capacity after each handoff because runtime limits may change. Light profile changes implementation depth, not delegation intensity: it uses the shared checkout without worktrees by default and relies on exact disjoint file ownership; runtime capacity is an environmental limit, never a light-profile policy.

Provide each worker only the source material, evidence, logs and checks needed for its bounded packet while the lead keeps compact contracts and handoffs. Do not fill context windows, clone identical briefs, assign overlapping work or repeat unchanged checks merely to consume capacity or tokens. Use a single implementation agent only for a trivial change or a feature that cannot be separated safely after contracts and file ownership are considered; record that concrete reason in the kickoff packet.
...
For an implementation spanning both frontend and backend, refine the dependency graph once the scaffold and shared contract are stable. Assign one worker to each independent ready surface whose files, inputs and runtime state can be separated safely, filling available capacity before the lead begins feature implementation. Stop splitting only when the remaining work is dependent, overlaps ownership or would make shared runtime state unsafe. If a higher-priority instruction or explicit user request forbids parallel delegation, record that authorisation constraint. Otherwise, only a recorded trivial or safely inseparable change permits a single implementation owner. Workers, researchers, validators and reviewers are leaf agents and never delegate.

Admit a worker only when its prerequisite contract is stable, write set is disjoint, runtime and test data can be isolated, result has an observable acceptance check, and the packet advances a mandatory criterion, removes a material uncertainty or provides required independent evidence without duplicating work or delaying the critical path. Duplicate attempts are reserved for hard diagnosis or alternative designs with an objective selector such as tests, measured output or an independent reviewer. Keep the lead's context to the brief, contracts, decisions, dependency graph, acceptance state and compact summaries; raw exploration, logs and stack traces stay with workers.

Use the strongest justified reasoning for contract decisions, difficult integration and high-risk review. Prefer faster lower-effort agents for bounded search, file mapping, repetitive checks, deterministic test execution and structured summaries. Spend available context on each packet's useful evidence and implementation detail; do not inflate effort or duplicate work without evidence that it improves the result.
...
Apply the dedicated validation-agent gate from the shared global instructions. Allocate separate frontend and backend validators whenever both surfaces are affected; each must be distinct from every implementation author, and a further independent reviewer assesses the settled change. In normal delivery, fill a freed slot first with any ready independent implementation or research packet; if none is ready, start the matching validator as soon as its surface and contract are stable. Do not wait for unrelated implementation work to settle. Rapid delivery follows the surface pipeline below. Run validators concurrently when slots, build outputs, services and test data are independent; otherwise run them sequentially, recycling worker slots as soon as each surface is ready. Dispatch the final reviewer immediately once the combined change and tests are settled and capacity is available. Give validators the original acceptance criteria and contract, not just the implementation summary.
