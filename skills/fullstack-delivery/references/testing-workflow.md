...
Implementation agents use test-first checks as feedback. They do not perform their own independent review. A fresh frontend validator and a fresh backend validator handle their affected surfaces; normal delivery may wait for implementation to settle, while rapid delivery pipelines each stable surface immediately. For a single-surface change, only that validator is needed. Give them the brief, acceptance criteria, contract, test environment, files, current commands and applicable validation guidance such as Playwright or UI guidelines. If needed, give them disjoint test-file ownership to add missing cases.

For rapid delivery, start each validator when its surface and contract inputs are stable instead of waiting for all implementation to finish. The validator runs the cheapest criterion-specific checks first and reports failures before expensive suites, then runs one full surface gate after fixes settle. Reuse a lead-managed long-lived server or preview, dependency installation and isolated fixture set; do not start competing builds or mutate shared test data concurrently. Assign the aggregate production build/static gate to one validator. The implementation owner fixes production defects and reports the focused rerun. The same validator then reruns failed and dependent checks while unaffected evidence for the recorded snapshot remains valid.
...
The lead coordinates shared builds, servers, dependencies and test data. Avoid concurrent builds into the same output directory or test runs that delete each other's fixtures. Recheck affected behavior after fixes. Report failures, skips and unavailable environments accurately; keep required cases failing rather than weakening them to make a report green. Evidence remains reusable while the tested files, dependencies, environment and contract are unchanged. After validation settles, the final read-only reviewer can run alongside an independent production build or deployment check.

Use compact defect handoffs: state/revision, severity, failed acceptance criterion, file and line or user-visible state, command or reproduction, expected result, observed result and owner. If the same blocking failure returns twice, the lead reassesses the contract, environment or root cause before another implementation-validation cycle.
